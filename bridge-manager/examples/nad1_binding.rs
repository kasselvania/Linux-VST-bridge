...
fn main() -> Result<(), Box<dyn std::error::Error + Send + Sync>> {
    ...
    if args
        .get(1)
        .is_some_and(|s| matches!(s.as_str(), "submit" | "stop" | "reconcile"))
    {
        use linux_vst_bridge::{dependency_session as lifecycle, Manager};
                ...
                let result = lifecycle::submit_with(
                    ...
                    |path| {
                        Ok(Command::new("systemd-run")
                            ...
                            .arg("/usr/bin/python3")
                            .arg("-B")
                            .arg(Path::new(&args[4]).join("owner_supervise.py"))
                            .arg(&args[5])
                            .arg(path)
                            .status()?
                            .success())
    ...
    spec["kind"] = serde_json::json!("native_access_dependency");
