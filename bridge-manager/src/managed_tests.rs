...
fn uir1_host_only_candidate_retains_ordinary_eleven_and_all_rollback_boundaries() {
    ...
    assert_eq!(sealed.requirements.native_sha256, ordinary.requirements.native_sha256);
    assert_eq!(external_ids(&sealed.class.class_id).unwrap(), external_ids(&ordinary.class.class_id).unwrap());
    assert_eq!(qualification::candidates_for(Qualification::Uir1Input).unwrap(), vec![sealed]);
    for boundary in BOUNDARIES.into_iter().map(Some).chain([None]) {
        ...
        let untouched = snapshot(prior.target.parent().unwrap());
        let mut candidate = p.clone();
        candidate.revision = 12; candidate.claim = Claim::ReviewCandidate;
        ...
        fs::copy(&n.artifact.path, package.join(format!("{}.so", p.class.class_id))).unwrap();
        ...
        qualification::stage_selected_for(&f.m, &package, &[candidate.clone()], Qualification::Uir1Input).unwrap();
        let staged = qualification::load_for(&f.m, candidate.clone(), Qualification::Uir1Input).unwrap();
        ...
        for mutation in 0..4 {
            ...
            match mutation {
                0 => wrong.requirements.native_sha256 = "ab".repeat(32),
                1 => wrong.capabilities.accessibility = Accessibility::WindowsDefault,
                2 => wrong.requirements.runner.version.push_str(" changed"),
                _ => wrong.revision = 13,
            }
            assert!(f.m.verify_qualification_parent_for(&f.m.registry().unwrap(), &wrong, &reg, Qualification::Uir1Input).is_err());
        }
        assert!(f.m.managed_publish(&candidate, &census, reg.clone(), &census.host, &census.host_source_sha256, None).is_err());
        let result = f.m.publish_selected(&candidate, &census, reg, (&census.host, &census.host_source_sha256), Some(Qualification::Uir1Input), boundary);
        if boundary.is_some() {
            assert!(result.is_err()); f.m.reconcile().unwrap();
        } else {
            let active = f.m.load_revision(&p.class.class_id, &result.unwrap()).unwrap();
            assert_eq!(active.parent, Some(parent.clone()));
            assert_eq!(active.qualification, Some(Qualification::Uir1Input));
            f.m.restore_editor_qualifications().unwrap();
        }
