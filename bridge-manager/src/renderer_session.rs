//! Exact renderer reservation, submission and no-writer recovery. No product authority.
use crate::*;
...
pub fn directory(m: &Manager) -> PathBuf {
    m.root.join("vendor-applications/native-access")
}
...
pub fn unit(op: &str) -> Result<String> {
    require(valid_hex(op, 32), "renderer_operation_identity")?;
    Ok(format!("linux-vst-bridge-renderer-{op}.service"))
}
...
pub fn exact(m: &Manager, op: &str) -> Result<()> {
    require(current(m)?["operation"] == op, "renderer_operation_changed")
}
...
fn immutable(p: &Path, v: &Value) -> Result<()> {
    ...
    installed?;
    File::open(p.parent().ok_or("renderer_parent")?)?.sync_all()?;
    Ok(())
}
pub fn inspect(op: &str) -> Result<UnitState> {
    ...
    let output = Command::new("systemctl")
        ...
        .output()?;
    require(output.stdout.len() <= 16384, "renderer_unit_query_bound")?;
    ...
    require(
        ...
        "renderer_unit_state_unavailable",
    )?;
    let load = *fields.get("LoadState").ok_or("renderer_unit_load")?;
    require(
        matches!(load, "loaded" | "not-found") && (output.status.success() || load == "not-found"),
        "renderer_unit_query_failed",
    )?;
    let group = format!(
    ...
    while let Some(path) = pending.pop() {
        ...
        count += 1;
        require(count <= 128, "renderer_cgroup_bound")?;
        let bytes = fs::read(path.join("cgroup.procs"))?;
        require(bytes.len() <= 65536, "renderer_members_bound")?;
...
fn close_empty(m: &Manager, op: &str, probe: &UnitState) -> Result<Submission> {
    ...
    if !prior.is_null() {
        require(
            prior["operation"] == op,
            "renderer_result_operation_changed",
        )?;
...
pub fn submit_with(
    ...
    let op = spec["operation"].as_str().ok_or("renderer_operation")?;
    let d = operation_dir(m, op)?;
    require(
        spec["report"] == d.join("result.json").to_string_lossy().as_ref(),
        "renderer_report_location",
    )?;
    private_dir(&directory(m).join("operations"))?;
    require(!d.exists(), "renderer_duplicate_operation")?;
    private_dir(&d)?;
    ...
    let operator_lock::LockAttempt::Acquired(_gate) = gate(m, op)? else {
        return Err("renderer_new_gate_busy".into());
    };
...
pub fn stop_with(
    ...
    {
        let operator_lock::LockAttempt::Acquired(_gate) = gate(m, op)? else {
            return Err("renderer_writer_still_active".into());
        };
        let observed = probe();
        record_probe(m, op, &observed)?;
    ...
    require(retired_with(m, op, probe)?, "renderer_cleanup_unconfirmed")
}
...
mod tests {
    ...
    fn stop_requires_exact_operation_and_preserves_interrupted_writer_evidence() {
        ...
        let prior = json!({"schema":1,"operation":op,"state":"running","renderer":{"facts":[{"category":"gpu"}]},"effective":{"policy":"inherited"}});
