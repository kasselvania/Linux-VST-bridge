...
## Active AP4 ruling — Actual plug-in state and Bitwig project recall

AP3 is accepted through PR #56; AP2, AP1, AP0 and PC0 remain accepted. The operator approved preparing and undertaking the next project-recall slice. The lead selects and authorizes **AP4** on `codex/ap4-plugin-state-project-recall`, based on `8e60bace2f685b94fb5936919069103e3dc9a376`. Issue #57 tracks it. `CURRENT_SLICE.md` is the complete bounded product decision, proof and execution allowance; it is not a request for another planning package.

One engineer owns implementation, necessary native/Windows builds, focused tests, exact source/artifact/runtime binding, fresh Deck/Bitwig verification and one implementation PR. This is an explicit PRODUCT_CONTRACT_CHANGE ruling. Real component-state get/set, bounded stream transport, correct Windows owner-thread servicing, reference-controller synchronization, host lifecycle integration and the saved disposable project are authorized. Narrow changes to the existing call audit, producer inputs, protocol/version, adapters, supervision and evidence needed for these operations are included. No separate design, maintenance, activation or pre-run approval handoff is required within the work order. Final review and merge remain separate.

This ruling supersedes historical prohibitions on state/preset calls, initialized-session access, state-related lifecycle changes and current-branch builds only as needed for AP4. It does not authorize vendor editor/controller proxying, commercial fixtures, a preset manager or general broker. Preserve accepted audio behavior, callback/worker separation, reported latency, actual Windows DSP and process/mapping ownership. Never invent plug-in state from a Linux gain cache, silently default after a failed restore, or move blocking state work into a callback. The work order closes observable behavior; private types, files and narrow implementation/refactoring choices belong to the engineer.

Reuse established Moonlight/Sunshine access under existing permissions and `docs/DECK_REMOTE_DESKTOP.md`; SSH remains the shell/build path. No reinstallation or re-pairing is needed for this task. No streamed terminal or bypass of Computer Use exclusions, authentication, permissions or sandboxing. Scoped temporary publication and saving/reopening the disposable test project are authorized fixture changes; preserve original settings, other projects and commercial installations. Establish comparison baselines before measurement, never to hide unexplained drift afterward.

Routine in-scope repairs, mechanical source/candidate binding and the work order's bounded runtime-update contingency are delegated. Preserve all consumed counts and prior evidence; do not relaunch or promote historical observations. AP3 remains the accepted frontier until AP4 review/merge. Continue to actual restored processing and project recall, not merely a serializer, successful build or another plan.
