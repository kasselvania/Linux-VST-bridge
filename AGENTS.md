...
## Completed AP3 ruling — Sustained audio and remote development access

AP3 is accepted through PR #56 after exact-head technical-lead review. The sustained reference-effect/Bitwig outcome and separate Sunshine/Moonlight operational outcome are complete. AP2, AP1, AP0 and PC0 remain accepted prerequisites. `CURRENT_SLICE.md` owns current status, the bounded accepted capability, evidence references and the next proposed product direction.

The completed AP3 work order does not authorize additional producer, diagnostic or acceptance workloads after merge. Preserve its fresh acceptance, all diagnostic failures/unknowns, same-reservation D5 recovery and cumulative counts. Do not repeat successful work for status closure, promote diagnostics, reset history or overwrite earlier evidence.

The established Mac-to-Deck desktop access remains a separate developer tool. Reuse it under existing shell, app and operator permissions as documented in `docs/DECK_REMOTE_DESKTOP.md`; do not reinstall or re-pair it unnecessarily. SSH remains the shell/build path. Never use a streamed terminal or remote desktop to bypass Computer Use exclusions, sandboxing, authentication or denied permissions. The retained service is not a leaked Windows product process or permission to launch new product workloads.

No implementation successor is active. Read-only investigation and review remain permitted; a subsequent work order must define the next product capability and its execution scope. Preserve callback/worker separation, reported added latency, process/mapping ownership and honest limits. The accepted measured preview is not general realtime, saved-state, vendor-editor or commercial compatibility.
