...
inline std::vector<uint8_t> encode(const Frame& f,uint16_t minor=1){
 require(minor>=1&&minor<=8&&f.kind>=Hello&&f.kind<=(minor>=6?Configured:minor>=4?StateApplied:minor>=2?Deactivated:Error)&&f.payload.size()<=(minor>=4&&f.kind>=State?(1u<<20):(minor==5||minor==7||minor==8)&&f.kind==Process?(minor==8?8344:8248):4040),"frame kind/length");
...
inline size_t payload_length(const uint8_t* b,uint16_t minor=1){
 require(get(b,4)==magic&&get(b+4,2)==1&&get(b+6,2)==minor&&(minor>=1&&minor<=8)&&get(b+10,2)==0,"protocol version/header");
 ...
 auto n=get(b+12,4);require(n<=(minor>=4&&get(b+8,2)>=State?(1u<<20):(minor==5||minor==7||minor==8)&&get(b+8,2)==Process?(minor==8?8344:8248):4040),"frame length");return size_t(n);
}
