...
inline size_t payload_length(const uint8_t* b,uint16_t minor=1){
 require(get(b,4)==magic&&get(b+4,2)==1&&get(b+6,2)==minor&&(minor>=1&&minor<=12)&&get(b+10,2)==0,"protocol version/header");
 ...
 auto n=get(b+12,4);require(n<=(minor>=4&&get(b+8,2)>=State?(1u<<20):minor>=9&&get(b+8,2)==Done?10312:(minor==5||minor==7||minor==8||minor==9||minor==10||minor==11||minor==12)&&get(b+8,2)==Process?(minor>=10?8352:minor>=8?8344:8248):4040),"frame length");return size_t(n);
}
