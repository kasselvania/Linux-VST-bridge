...
struct BusLayout {
 ...
 std::array<Bus,AP18Buses::max_buses> buses{};size_t size=0;std::array<int,4> counts{};int transported_input=-1;
 void read(Steinberg::Vst::IComponent& c,Steinberg::Vst::IAudioProcessor& p,bool zero_unspecified=event_output_policy()){
  ...
  for(int media=0;media<2;++media)for(int dir=0;dir<2;++dir){auto n=c.getBusCount(media,dir);require(n>=0&&n<=AP18Buses::capacity(media,dir),"bus count bound");counts[media*2+dir]=n;
   for(int i=0;i<n;++i){auto& b=buses[size++];require(c.getBusInfo(media,dir,i,b.info)==kResultOk,"bus metadata query");
    ...
    if(media==kAudio){require(p.getBusArrangement(dir,i,b.arrangement)==kResultOk&&b.arrangement==SpeakerArr::kStereo&&b.info.channelCount==2,"only declared stereo audio supported");require(dir!=kInput||b.info.busType!=kMain||i==0,"main audio input index");require(dir!=kOutput||i!=0||b.info.busType==kMain,"index-zero main audio output required");}
  ...
  require(counts[1]>=1&&counts[2]<=1,"main output and optional event input required");
 }
 void contract(const std::vector<uint8_t>& p){
  ...
  for(size_t i=0;i<size;++i){const auto*r=p.data()+28+32*i;auto&b=buses[i];auto m=b.info.mediaType,d=b.info.direction;
   ...
   auto enabled=get(r+20,4);require(enabled<=1&&(!enabled||b.supported),"bus has no audio transport lane");b.active=enabled!=0;
  }
 }
 ...
 // Preserve bus indices and channel-pointer arrays for inactive outputs, as
 // required by AudioBusBuffers. Inactive sample addresses may be null.
 void map_outputs(std::array<Steinberg::Vst::AudioBusBuffers,AP18Buses::max_audio_outputs>& outputs,
                  float** transported,float** inactive)const {
  int index=0;
  for(size_t i=0;i<size;++i){const auto& b=buses[i];
 ...
 void negotiate(Steinberg::Vst::IAudioProcessor& p){
  using namespace Steinberg;using namespace Steinberg::Vst;std::array<SpeakerArrangement,AP18Buses::max_audio_inputs> in{};std::array<SpeakerArrangement,AP18Buses::max_audio_outputs> out{};int ni=0,no=0;
