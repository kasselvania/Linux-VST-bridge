...
struct GraphOwner {
  ...
  void open(const wchar_t* output,DWORD pid,DWORD tid,uint64_t born,HWND root){
    ...
    memcpy(graph->magic,"UIO2",4);graph->version=2;graph->bytes=sizeof(uio2::Graph);graph->row_bytes=sizeof(uio2::Window);
...
struct Owner {
  ...
  HMODULE dll=nullptr;HHOOK hooks[5]{};uio1::Header* h=nullptr;
  ~Owner(){
    if(h){
...
int observe(HWND root,uint64_t expected,unsigned seconds,const wchar_t* output,bool surfaces=false,bool input=false){
...
int wmain(int argc,wchar_t**argv){try{
  ...
  if(argc==6&&wcscmp(argv[1],L"observe-input")==0)return observe(reinterpret_cast<HWND>(_wcstoui64(argv[2],nullptr,0)),_wcstoui64(argv[3],nullptr,0),unsigned(wcstoul(argv[4],nullptr,10)),argv[5],false,true);
