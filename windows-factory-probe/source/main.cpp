...
std::map<std::string, std::string> parse_args(int argc, char** argv) {
    ...
    if (!is_lower_hex(result["--session"], 32) ||
        ...
        (result["--mode"] != "wa0-audio-processor-interface-admission" &&
         ...
         result["--mode"] != "ap3-queued-audio-preview" && result["--mode"] != "ap4-plugin-state-recall" && result["--mode"] != "ap18-bus-lifecycle" && result["--mode"] != "ap18-stereo-negotiation" && result["--mode"] != "ap8-module-inspection" && result["--mode"] != "ap8-commercial-preview" && result["--mode"] != "ap9-reference" && result["--mode"] != "ap9-commercial" && result["--mode"] != "ap12-vendor-access") ||
        ((result["--mode"] == "ap18-bus-lifecycle" || result["--mode"] == "ap18-stereo-negotiation" || result["--mode"] == "ap8-module-inspection" || result["--mode"] == "ap8-commercial-preview" || result["--mode"] == "ap9-commercial" || result["--mode"] == "ap12-vendor-access")
...
int main(int argc, char** argv) {
    ...
    try {
        ...
        const bool stereo_probe=args.at("--mode")=="ap18-stereo-negotiation";
        if (access_mode || ap8_mode || args.at("--mode") == "ap18-bus-lifecycle" || stereo_probe || args.at("--mode") == "ap8-module-inspection") {
        ...
        if (factory != nullptr && primary == 0 && (args.at("--mode")=="ap18-bus-lifecycle"||stereo_probe||args.at("--mode")=="ap8-module-inspection"||ap8_mode||access_mode)) {
            events.lifecycle("ap8_factory", wf0::census_json_fields(census.census));
            primary=wf0::inspect_module(factory,events,args.at("--component-case")=="first-audio"?"":args.at("--component-case").substr(6),mapped.get(),access_mode?ready_path.substr(0,ready_path.find_last_of(L"\\/")):L"",args.at("--mode")=="ap18-bus-lifecycle",stereo_probe);
            if(first_primary==0)first_primary=primary;
        ...
        if((args.at("--mode")=="ap18-bus-lifecycle"||stereo_probe||args.at("--mode")=="ap8-module-inspection"||ap8_mode||access_mode)) {
