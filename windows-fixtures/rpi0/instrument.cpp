...
class Controller final:public EditControllerEx1 {
 ...
 public:
  ...
  tresult PLUGIN_API setParamNormalized(ParamID id,ParamValue value)override{if(id==gain_parameter&&std::isfinite(value)&&value>=0&&value<=1)gain_=value;return EditControllerEx1::setParamNormalized(id,value);}
...
tresult Editor::attached(void*parent,FIDString type){if(isPlatformTypeSupported(type)!=kResultOk||!parent||child_)return kResultFalse;WNDCLASSW wc{};wc.lpfnWndProc=window;wc.hInstance=GetModuleHandleW(nullptr);wc.lpszClassName=L"LVB_RPI0_Editor";wc.hCursor=LoadCursorW(nullptr,IDC_ARROW);wc.style=CS_DBLCLKS;if(!RegisterClassW(&wc)&&GetLastError()!=ERROR_CLASS_ALREADY_EXISTS)return kResultFalse;systemWindow=parent;child_=CreateWindowExW(0,wc.lpszClassName,L"",WS_CHILD|WS_VISIBLE,0,0,rect.getWidth(),rect.getHeight(),HWND(parent),nullptr,wc.hInstance,this);return child_?kResultOk:kResultFalse;}
tresult Editor::removed(){if(child_&&IsWindow(child_))DestroyWindow(child_);child_=nullptr;systemWindow=nullptr;return kResultOk;}
