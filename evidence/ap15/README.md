...
`native-artifacts.json` retains the initial Linux experiment and both descriptor-specific proxy build receipts from source commit `48e52a84ed21657c5138bb89c5268a38e09a8d6c`. The unchanged descriptor hashes preserve the existing class/parameter/external-ID generation inputs. These artifacts remain in the build cache and were not published. `ci-start-condition.json` retains all four source-head jobs, each with no assigned runner or steps and GitHub's billing/spending annotation.
