...
samples per channel, but the counters do not establish that current audio
resumed after recovery. Final callback totals recorded 4,608 missing frames
...
STOP/START portion executed. Fault 7 then records another live-lateness
incident during or after that recovery; it does not establish a playable
return to current input. The retained log does not establish a vendor
exception or Windows-host crash before the owner stopped the service.
...
This is a **bounded commercial failure**, not a Serum usability or recovery
success. BR1 prevented the prior 2,048-request stale backlog and exposed a
specific exhausted-recovery classification, but one preset change still
