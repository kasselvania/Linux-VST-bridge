#!/usr/bin/env python3
"""Assemble the one sealed IF1 host package from its exact hosted archive.
Locations supply bytes, never profile policy. The native is bound to the IF1 source receipt.
"""
import argparse
import hashlib
import json
...
def material(windows_zip):
    ...
    manifest = dict(schema='linux-vst-bridge-if1-host-source/v1',
