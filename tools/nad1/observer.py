...
def main():
    ...
    process=proc_census(env/'compatdata/pfx');private_process=process.pop('private');ports=listeners()
    ...
    publish(out/'private.json',{'registry':regraw.decode(),'application':app,'software':sw,'processes':process,'candidate_failures':private_process,'source_files':{k:v.decode('utf8','replace') for k,v in private.items()}})
