...
"""Bounded Unix-socket service for the Fates 128x64 SSD1322 OLED.

The SSD1322 initialization sequence, address window, and one-byte-per-logical-pixel
wire mapping are adapted from monome/linux fb_ssd1322.c at commit
458e2253667a09bd01f1d50cc2b4063c14c491ec. The service/control protocol and bounded
renderer are original FATES0 code. See THIRD_PARTY_NOTICES.md.
"""
...
class Ssd1322:
    def __init__(self, spi_path: Path, rotate: int) -> None:
        ...
        self.lines = gpiod.request_lines(
            chip_path,
            consumer="fates0-oled",
            config={17: settings, 4: settings},
        )
        self.gpiod = gpiod
...
def main() -> int:
    ...
    parser.add_argument("--socket", type=Path, default=Path("/run/fates0/oled.sock"))
    parser.add_argument("--spi", type=Path, default=Path("/dev/fates-oled-spi"))
