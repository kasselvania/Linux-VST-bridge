...
service="fates-jack@$target_user.service"
env_file=$FATES0_CONFIG_DIR/jack.env
[[ -f $env_file ]] || die "missing JACK environment file: $env_file"
saved_env=$(mktemp /var/tmp/fates0-jack-env.XXXXXX)
...
for period in 128 256 512 1024; do
  cat >"$env_file" <<EOF
FATES_SAMPLE_RATE=48000
FATES_PERIOD_FRAMES=$period
FATES_PERIODS=3
EOF
