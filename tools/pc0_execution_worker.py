...
def schema_for(binding):
    if binding.get("product") in {"AP0", "AP1", "AP2", "AP3"}:
...
def preflight(binding):
    ...
    if binding.get("product") in {"AP1", "AP2", "AP3"}:
    ...
    if is_acceptance(binding) or binding.get("product") in {"AP0", "AP1", "AP2", "AP3"}:
...
def execute(proof, binding):
    ...
    def checkpoint(stage, available=None, error=None):
        ...
        temporary = None
        try:
            if len(raw) > (512 if binding.get("product") in {"AP2", "AP3"} else 256 if binding.get("product") == "AP1" else 128) * 1024:
