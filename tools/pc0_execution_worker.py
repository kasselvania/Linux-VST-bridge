...
def schema_for(binding):
    if binding.get("product") in {"AP0", "AP1", "AP2", "AP3", "AP4"}:
...
def preflight(binding):
    ...
    if binding.get("product") in {"AP1", "AP2", "AP3", "AP4"}:
    ...
    if binding.get("product") in {"AP0", "AP1", "AP2", "AP3", "AP4"}:
        if binding.get("product") == "AP4":
            from ap4_worker_support import verify_host, bind_client, configure
            configure(binding["ap4_gui"])
            bind_client(binding["native_client"])
        elif binding.get("product") == "AP3":
            from ap3_worker_support import verify_host, bind_client
            bind_client(binding["native_client"])
        elif binding.get("product") == "AP2":
    ...
    if is_acceptance(binding) or binding.get("product") in {"AP0", "AP1", "AP2", "AP3", "AP4"}:
...
def execute(proof, binding):
    ...
    if binding.get("product") in {"AP0", "AP1", "AP2", "AP3", "AP4"}:
        if binding.get("product") == "AP4":
            import ap4_worker_support as ap0
        elif binding.get("product") == "AP3":
            import ap3_worker_support as ap0
        elif binding.get("product") == "AP2":
    ...
    def checkpoint(stage, available=None, error=None):
        ...
        temporary = None
        try:
            if len(raw) > (512 if binding.get("product") in {"AP2", "AP3", "AP4"} else 256 if binding.get("product") == "AP1" else 128) * 1024:
