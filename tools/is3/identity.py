"""Closed, canonical IS3 package and experiment identity. Paths stay private."""
import hashlib,json,os,pathlib,re,stat,struct,tempfile

SOURCES={name:'tools/is3/'+name for name in ('identity.py','package.py','campaign.py','supervise.py','run.py','report.py','environment.py','comparison.py','capability.cpp')}
...
def atomic_new(path,value):
    ...
    fd,tmp=tempfile.mkstemp(prefix='.is3-',dir=path.parent)
    try:
...
def verify_package(root,seal,source):
    ...
    exact(m['build'],('recipe','compiler_sha256','compiler_version'));sha(m['build']['compiler_sha256'])
    if m['build']['recipe']!='zig-x86-cpp20-bcrypt-v1' or len(m['build']['compiler_version'])>128:raise ValueError('build_recipe')
