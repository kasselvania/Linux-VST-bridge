...
def run(payload, output):
    ...
    subprocess.run(['systemd-run','--user','--collect','--property=UMask=0077','--property=KillMode=control-group','--property=TimeoutStopSec=20','--property=StandardOutput=null','--property=StandardError=null','--unit='+unit,'/usr/bin/python3',software['supervisor']['path'],'--install',str(spec)],check=True,timeout=15)
    try:
    ...
    lines=[l.decode() for l in log.read_bytes().splitlines() if l.startswith(b'IS3_CAP_V1')]
    images={}
...
if __name__=='__main__':
    p=argparse.ArgumentParser();p.add_argument('--payload',type=pathlib.Path,required=True);p.add_argument('--output',type=pathlib.Path,required=True);a=p.parse_args();run(a.payload.resolve(),a.output.resolve())
