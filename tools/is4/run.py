...
def run(package, output, mode, seal, source):
    ...
    proof['policy']=read_json(output/(operation+'-installer-policy.private.json'))
    proof['policy_binding_sha256']=digest(output/'bound-spec.private.json')
    bound=read_json(output/'bound-spec.private.json')['installer_capability']
    ...
    generation=hashlib.sha256(json.dumps(generated,sort_keys=True,separators=(',',':'),ensure_ascii=False).encode()).hexdigest()
    if bound['software_sha256']!=generation or proof['policy']['software_sha256']!=generation:
        raise ValueError('production_generation_drift')
    proof['policy_owner_digests']=expected_owners
