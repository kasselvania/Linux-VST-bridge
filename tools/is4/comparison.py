...
def compare(proofs):
    ...
    for p in proofs:
        ...
        if u['operation']!=p['binding']['operation'] or u['fixture_sha256']!=p['payload_sha256'] or u['phase']!='target_runner_after_prefix_initialization' or u['mode']!='baseline':raise ValueError('unix_identity')
    ...
    if len({p['policy']['software_sha256'] for p in proofs})!=1:raise ValueError('policy_generation_drift')
    for p in proofs:
        ...
        if expected=='inherited':
            if after!=policy['before']:raise ValueError('inherited_changed')
        elif after!={'present':True,'length':15,'sha256':'a156d13bd0ad6129fa13a0a7b76672634c09956c9b090e7c89edf60078979c63'}:raise ValueError('effective_policy')
    mid=proofs[1]
    ...
    return {'schema':2,'comparison_identity_sha256':hashed(proofs[0]['identity']),'disposition':'IS4_OPERATION_SCOPED_HONEST_ABSENCE_PROVED' if absence else 'IS4_UNIX_UNAVAILABILITY_NOT_ESTABLISHED',
