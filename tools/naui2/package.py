...
SOURCES.update({'launch.cpp':'tools/is2/launch.cpp','binding.rs':'bridge-manager/examples/naui2_binding.rs','application.rs':'bridge-manager/src/renderer_application.rs'})
...
def build(repo,out,context):
    ...
    with tempfile.TemporaryDirectory(prefix='naui2-build-') as d:
        for name,src,libs in [('payload.exe','application.cpp',[]),('adapter.exe','launch.cpp',['-lbcrypt'])]:
