...
from ap2_native_artifact import PATHS,NAMES,canonical,digest,verify_native
...
def main():
 ...
 key=digest(archive.read_bytes());ssh=transfer_port();parent='/home/deck/.cache/linux-vst-bridge/ap2-builds'
...
parent=pathlib.Path.home()/'.cache/linux-vst-bridge/ap2-builds';source=parent/KEY;archive=parent/(KEY+'.zip')
...
commands=[['cmake','-S',str(source),'-B',str(source/'build'),'-G','Ninja','-DCMAKE_BUILD_TYPE=Release','-DAP2_BUILD_ONLY=ON','-DVST3_SDK_ROOT='+str(sdk),'-DAP2_RUST_LIBRARY='+str(source/'libap2_backend.a')],['cmake','--build',str(source/'build'),'--target','AGainOfflineBridge','ap2-offline-host','-j','4']]
...
'''
 ...
 (root/'AP2_NATIVE_BUILD.json').write_bytes(canonical(record));verify_native(root,binding)
