...
int main(){
  ...
  for(uint64_t i=0;i<count;++i){assert(r[i].commit==i+1&&r[i].hwnd!=uint64_t(foreign.load()));if(r[i].message==WM_LBUTTONDOWN){assert(r[i].hwnd==uint64_t(child.load())&&r[i].x==15&&r[i].y==19&&r[i].action==17);get+=r[i].source==1;}if(r[i].message==WM_SIZE){enter+=r[i].source==2;leave+=r[i].source==3;}if(r[i].source==4){printf("heartbeat ticks=%lld frequency=%llu\n",r[i].result,shared->frequency);if(r[i].result>int64_t(shared->frequency/5))slow=true;}}
