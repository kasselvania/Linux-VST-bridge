...
int main(){
  ...
  std::thread ui([]{
    ...
    ShowWindow(p,SW_SHOW);SetForegroundWindow(p);SetFocus(child);
    auto filter=SetWindowsHookExW(WH_MOUSE,mouse_filter,nullptr,GetCurrentThreadId());assert(filter);
    parent=p;MSG m{};while(GetMessageW(&m,nullptr,0,0)>0){TranslateMessage(&m);DispatchMessageW(&m);}UnhookWindowsHookEx(filter);DestroyWindow(foreign);DestroyWindow(p);
  });
  ...
  SendMessageW(child,WM_SIZE,0,MAKELPARAM(160,120));
  POINT pt{40,40};assert(ClientToScreen(child,&pt));assert(SetCursorPos(pt.x,pt.y));
  assert(GetForegroundWindow()==parent);
  INPUT clicks[2]{};for(auto&i:clicks)i.type=INPUT_MOUSE;clicks[0].mi.dwFlags=MOUSEEVENTF_LEFTDOWN;clicks[1].mi.dwFlags=MOUSEEVENTF_LEFTUP;
  ...
  for(uint64_t i=0;i<count;++i){assert(r[i].commit==i+1&&r[i].hwnd!=uint64_t(foreign.load()));if(r[i].message==WM_LBUTTONDOWN&&r[i].x==15&&r[i].y==19){assert(r[i].hwnd==uint64_t(child.load())&&r[i].action==17);get+=r[i].source==1;}
