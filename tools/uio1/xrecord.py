...
class Recorder:
    def __init__(self, x, capacity=4096):
        ...
        ranges=self.ranges();pointers=(C.POINTER(Range)*len(ranges))(*(C.pointer(r) for r in ranges));client=U(x.window)
        self.context=t.XRecordCreateContext(x.display,1,C.byref(client),1,pointers,len(ranges));x.sync()
