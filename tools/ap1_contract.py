...
LENGTHS=[1,16,63,256,16,63,1,256]
GAINS=[.5,.25,.75,.5,.75,.25,.5,.75]
...
def input_words(seed,b,ch):
    ...
    for i in range(LENGTHS[b]):
        ...
        value=0. if b==5 else (x%129-64)/128
...
def compare_caller(caller):
    ...
    records=caller['records'];require(len(records)==10,'native record count')
    ...
    errors=[]
    for b,record in enumerate(records[1:9]):
        require(record['event']=='ap1_client_block' and record['sequence']==b+1 and record['frames']==LENGTHS[b] and record['gain']==GAINS[b] and record['silent'] is (b==5),'native request correlation')
    ...
    closed=records[-1];require(closed['event']=='ap1_client_closed' and closed['blocks']==8 and closed['samples_compared']==len(errors) and closed['maximum_absolute_error']==0. and closed['closed_received'] is True and closed['mapping_unmapped'] is True and closed['replays']==0,'native close/comparison')
    return {'samples_compared':len(errors),'blocks':8,'maximum_absolute_error':max(errors),'comparison_rule':'independent Linux request recipe; exact equality; tolerance zero'}

def normalize(observed):
    summary=validate_lifecycle(observed,expected_blocks=8)
    ...
    require(len(mapping)==len(closed)==1 and [r['block'] for r in buffers]==list(range(8)),'Windows mapping/close/private buffers')
