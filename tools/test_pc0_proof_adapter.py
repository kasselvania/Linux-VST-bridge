    ...
    def run(self, argv, *, cwd=None, timeout=30.0, input_bytes=None):
        ...
        if command == ("git", "rev-parse", "HEAD"):
            ...
            if target == f"{STOPPED_PC0_SOURCE}^{{tree}}":
                ...
                value = adapter_module._git_blob_sha1(adapter_module.WORKER_SOURCE.encode())
            elif any(target.endswith(":tools/"+name+".py") for name in adapter_module.SUPPORT_SOURCES):
                name=target.split("/")[-1].removesuffix(".py")
                value=adapter_module._git_blob_sha1(adapter_module.SUPPORT_SOURCES[name].encode())
...
class PC0AdapterTests(unittest.TestCase):
    ...
    def setUp(self):
        ...
        from test_pc0_diagnostic_runtime import reference_observation
        self.runtime_observation=reference_observation()
        import pc0_diagnostic_runtime as diagnostic_runtime
        self.stack.enter_context(patch.object(diagnostic_runtime, "_pc0_source_sha256",
            sha256_bytes(adapter_module.SUPPORT_SOURCES["pc0_diagnostic_runtime"].encode()),create=True))
        self.mock(diagnostic_runtime, "verify_diagnostic_runner", return_value=self.runtime_observation)
        import types
        diagnostic=types.ModuleType("pc0_diagnostic_primitives")
        diagnostic.__file__="<pc0_diagnostic_primitives>"
        diagnostic._pc0_source_sha256=sha256_bytes(adapter_module.SUPPORT_SOURCES["pc0_diagnostic_primitives"].encode())
        exec(compile(adapter_module.SUPPORT_SOURCES["pc0_diagnostic_primitives"],diagnostic.__file__,"exec"),diagnostic.__dict__)
        self.stack.enter_context(patch.dict(sys.modules,{"pc0_diagnostic_primitives":diagnostic}))
        self.diagnostic_runtime=diagnostic_runtime
        ...
        self.actual_build = self.run_module._build_for_normalizer
        self.mock(self.run_module, "_build_for_normalizer", return_value={})
        self.actual_normalizer = self.diagnostic.normalize_wa0_positive
        self.normalizer = self.mock(self.diagnostic, "normalize_wa0_positive", return_value=({"processing_contract": {}}, {}, {}))
    ...
    def scan(self, environment, *, mode, checkpoint=None):
        ...
        return {**diagnostic, "run_id": f"{self.launches:032x}",
                "raw_exit": None if self.failure else 0,
                ...
                "runner_identity": self.runtime_observation,
                "records": []}
    ...
    def test_real_supervisor_exception_preserves_cause_and_owned_cleanup(self):
        from types import SimpleNamespace
        from unittest.mock import MagicMock
        environment = SimpleNamespace(session=self.base, run_id="a"*32, marker={"fixture":"again"})
        root = MagicMock(pid=123, returncode=-15)
        selector = MagicMock()
        captures = []
        def pump_error(selector, streams, timeout):
            streams.records.append({"event":"call_started", "operation":"load_library"})
            raise ValueError("stream framing failed")
        def capture(stage, available, error):
            captures.append((stage, copy.deepcopy(available), self.diagnostic_runtime.exception_detail(error)))
        with contextlib.ExitStack() as stack:
            for name,value in {
            ...
            stack.enter_context(patch.object(self.diagnostic.subprocess,"Popen",return_value=root))
            stack.enter_context(patch.object(self.diagnostic.selectors,"DefaultSelector",return_value=selector))
            stack.enter_context(patch.object(self.diagnostic,"pump",side_effect=pump_error))
            observed = self.actual_supervise(environment,mode=self.common.PC0_MODE,checkpoint=capture)
        cleanup.assert_called_once_with(root,[(123,1)])
        selector.close.assert_called_once()
        self.assertEqual(observed["classification"],"supervision_failed")
        self.assertEqual(observed["supervision_exception"][0]["type"],"ValueError")
        self.assertEqual(captures[0][1]["records"][-1]["operation"],"load_library")
        self.assertEqual(captures[0][2][0]["frames"][-1]["function"],"pump_error")
        self.assertEqual(captures[-1][1]["cleanup"],{"owned_descendants_zero":True,"process_group_empty":True})
    ...
    def test_unknown_consumed_then_late_exact_result_one_launch(self):
        ...
        def delayed(*args, **kwargs):
            try:
                ...
                for path in self.remote_proof.glob("diagnostics/*/*/*/*"):
                    if path.name not in {"observation.json", "observation.sha256", "checkpoint.json"}:
                        continue
    ...
    def test_unknown_supervision_never_relaunches(self):
        ...
        self.assertEqual((first.state, second.state), ("CLOSED", "CLOSED"))
        self.assertEqual(self.launches, 1)
    ...
    def test_failed_retirement_is_recorded_truthfully(self):
        ...
        publication = next(self.remote_proof.glob("diagnostics/*/*/*/observation.json"))
        value = json.loads(publication.read_bytes())
        self.assertEqual(value["summary"]["troubleshooting"]["retirement"], "failed")
    ...
    def complete_synthetic_scan(self):
        ...
        callbacks = [
            {"operation":"addRef","origin":"component","enclosing_operation":"initialize_component",
             "enclosing_attempt_sequence":1,"reference_count":2},
            {"operation":"release","origin":"component","enclosing_operation":"terminate_component",
             "enclosing_attempt_sequence":2,"reference_count":1},
            {"operation":"release","origin":"owner_local","enclosing_operation":None,"reference_count":0}]
        session = {"state":"component_released","primary_blocker":None,
            "processor_cid_raw_tuid_hex":"5FDEE8845592534F96FAE4133C935A18",
            "requested_iid_raw_tuid_hex":"31FF31E8D5F20143928EBBEE25697802",
            "create":{"attempted":True,"result_u32_hex":"00000000","output_nonnull":True,"tuple_consistent":True},
            "controller_id":{"attempted":True,"buffer_zero_initialized":True,"result_u32_hex":"00000000",
                             "raw_tuid_hex":"655B9DD3AFD7FA42843F4AC841EB04F0","matches_expected":True},
            "initialize":{"attempted":True,"result_u32_hex":"00000000","succeeded":True},
            "terminate":{"attempted":True,"returned_ordinary":True,"result_u32_hex":"00000000"},
            "release":{"attempted":True,"returned_ordinary":True,"reference_count":0,"pointer_cleared":True},
            "host":{"created":True,"name":"Linux VST Bridge WC0","reference_baseline":1,
                    "reference_after_initialize":2,"reference_after_terminate":1,
                    "reference_returned_to_baseline":True,"owner_release_attempted":True,"owner_release_result":0},
            "audio_processor_session_ran":True,"audio_processor_lease":audio,
            "component_call_in_flight":False,"callback_ledger_closed":True,"object_quiescence":True,
            "inherited_shutdown_permitted":True,"processing_contract":contract,
            "callbacks":{"capacity":64,"record_count":3,"closed":True,"overflowed":False,
                "output_failed":False,"wrong_thread":False,"unexpected_object_request":False,
                "callback_in_flight":False,"records":callbacks}}
        states = list(n.EXPECTED_LIFECYCLE)
        i = states.index("audio_processor_release_in_flight")
        states[i:i] = ["bus_counts_validated","bus_info_complete","speaker_arrangements_complete","pre_setup_contract_complete"]
        coordinates = [{"operation":op} for op in n.EXPECTED_CALLS]
        coordinates[14:14] = copy.deepcopy(self.common.PC0_CALL_COORDINATES)
        records = []
        for coord in coordinates:
            interface = "IComponent" if coord["operation"] in {"get_bus_count", "get_bus_info"} else "IAudioProcessor"
            start = {"event":"call_started","sequence":len(records)+1,"interface":interface,**coord}
            result = {"event":"call_completed","sequence":len(records)+2,
                "attempt_sequence":start["sequence"],"interface":interface,**coord,"result_u32_hex":"00000000"}
            if coord["operation"] == "get_bus_count":
                result["i32_result"] = 0 if (coord["media_type"],coord["direction"]) == ("kEvent","kOutput") else 1
            records.extend([start,result])
        records.extend({"event":"host_callback","sequence":len(records)+i+1,**v} for i,v in enumerate(callbacks))
        for state in states:
            record = {"event":"lifecycle","sequence":len(records)+1,"state":state}
            if state == "pre_setup_contract_complete": record["processing_contract"] = contract
            records.append(record)
        def raw_id(logical):
            raw=bytes.fromhex(logical)
            return (raw[:4][::-1]+raw[4:6][::-1]+raw[6:8][::-1]+raw[8:]).hex().upper()
        final = records[-1]
        final.update(create_instance_called=True,controller_instance_created=False,
            audio_processor_interface_queried=True,audio_processor_method_called=True,
            component_session=session,class_count=3,
            factory={"vendor_hex":b"Steinberg Media Technologies".hex(),
                     "url_hex":b"http://www.steinberg.net".hex(),
                     "email_hex":b"mailto:info@steinberg.de".hex(),"flags_i32":16},
            classes=[{"raw_tuid_hex":raw_id(v[0]),"tier":"IPluginFactory3.PClassInfoW",
                      "name_hex":v[1].encode("utf-16-le").hex(),"category_hex":v[2].encode().hex()}
                     for v in n.EXPECTED_CLASSES])
        self.source.update(schema="local-test-source",record_count=0,records=[],tree="a"*40)
        self.host["manifest"] = {"records":[{"path":"bin/wf0-factory-probe.exe","sha256":"b"*64}]}
        self.run_module._build_for_normalizer.side_effect = self.actual_build
        self.mock(self.run_module, "dx0_source_role", return_value=dict(STOPPED_SOURCE_ROLE))
        def scan(*args, **kwargs):
            observed = self.scan(*args, **kwargs)
            observed.update(records=records, component_case="exact-again", raw_exit=0)
            return observed
    ...
    def test_complete_synthetic_observation_uses_real_normalizer_and_worker(self):
        ...
        self.assertEqual(observation["payload"]["diagnostic_data"]["processing_contract"], contract)
        self.assertEqual(self.launches, 1)
        self.retire.assert_called_once()
        # Same realistic input still enforces product assertions; no bypass.
        broken = copy.deepcopy(scan(object(),mode=self.common.PC0_MODE))
        broken["records"][-1]["component_session"]["audio_processor_lease"]["pointer_cleared"] = False
        with self.assertRaisesRegex(RuntimeError, "lifecycle differs"):
            self.actual_normalizer(broken,self.actual_build(self.source,self.host,self.fixture),pre_setup=True)

    def retained_trouble(self, receipt):
        return self.transaction(receipt)["observation"]["payload"]["diagnostic_data"]["troubleshooting"]

    def test_checkpoint_precedes_normalization_and_retirement(self):
        def normalize(*args, **kwargs):
            checkpoint = next(self.remote_proof.glob("diagnostics/*/*/*/checkpoint.json"))
            value = json.loads(checkpoint.read_bytes())["observation"]
            self.assertEqual(value["summary"]["troubleshooting"]["observation"]["classification"], "scanner_completed")
            self.retire.assert_not_called()
            return {"processing_contract": {}}, {}, {}
        self.normalizer.side_effect = normalize
        receipt = self.execute()
        self.assertEqual(self.transaction(receipt)["observation"]["kind"], "SUCCESS")
        self.retire.assert_called_once()

    def test_normalization_and_retirement_errors_preserve_original_observation(self):
        self.normalizer.side_effect = NameError("expected_calls is not defined")
        self.failed_retirement = True
        receipt = self.execute()
        trouble = self.retained_trouble(receipt)
        self.assertEqual(trouble["primary_error"]["stage"], "normalize")
        self.assertEqual(trouble["primary_error"]["exception"][0]["type"], "NameError")
        self.assertEqual(trouble["observation"]["classification"], "scanner_completed")
        self.assertEqual(trouble["retirement"], "failed")
        self.assertEqual([v["stage"] for v in trouble["secondary_errors"]], ["retirement", "failure_report"])
        checkpoint = next(self.remote_proof.glob("diagnostics/*/*/*/checkpoint.json"))
        retained = json.loads(checkpoint.read_bytes())["observation"]["summary"]["troubleshooting"]
        self.assertEqual(retained, trouble)

    def test_failure_validator_exception_has_independent_fallback(self):
        self.failure = True
        self.mock(self.diagnostic, "validate_failure_diagnostic", side_effect=ValueError("failure report schema differs"))
        receipt = self.execute()
        trouble = self.retained_trouble(receipt)
        self.assertEqual(trouble["primary_error"]["stage"], "failure_report")
        self.assertIn("schema differs", trouble["primary_error"]["exception"][0]["detail"])
        self.assertEqual(trouble["observation"]["classification"], "supervision_failed")
        self.assertEqual(trouble["retirement"], "retired")

    def test_supervisor_post_cleanup_exception_keeps_event_and_retires_owned_stage(self):
        def scan(environment, *, mode, checkpoint):
            self.launches += 1
            checkpoint("supervisor_containment", {"records": [{"event":"call_completed",
                "operation":"free_library", "bool_result":True}],
                "cleanup":{"owned_descendants_zero":True,"process_group_empty":True}})
            raise RuntimeError("protected readback failed at /home/private/data token=hidden")
        self.supervise.side_effect = scan
        receipt = self.execute()
        trouble = self.retained_trouble(receipt)
        self.assertEqual(trouble["last_event"]["operation"], "free_library")
        self.assertIn("protected readback failed", str(trouble["primary_error"]))
        self.assertNotIn("/home/private", str(trouble))
        self.assertNotIn("token=hidden", str(trouble))
        self.retire.assert_called_once()
        self.assertEqual(self.transaction(receipt)["observation"]["cleanup_disposition"], "COMPLETE")

    def test_publication_failures_and_ack_loss_reconcile_checkpoint_without_launch(self):
        original = pathlib.Path.open
        for filename in ("observation.json", "observation.sha256"):
            with self.subTest(filename=filename):
                def fail(path, *args, **kwargs):
                    if path.name == filename and args and args[0] == "xb":
                        raise OSError("injected publication failure")
                    return original(path, *args, **kwargs)
                source = SOURCE_A if filename.endswith("json") else SOURCE_B
                self.local_worker.lose_ack = True
                with patch.object(pathlib.Path, "open", fail):
                    receipt = self.execute(source)
                trouble = self.retained_trouble(receipt)
                self.assertEqual(trouble["primary_error"]["stage"], "publication")
                self.assertEqual(trouble["observation"]["classification"], "scanner_completed")
                self.assertEqual(trouble["retirement"], "retired")
    ...
    def test_checkpoint_persistence_failure_keeps_earlier_bytes_and_reports_error(self):
        import os
        original = os.replace
        saved = []
        def fail(src, dst):
            if pathlib.Path(dst).name == "checkpoint.json":
                if saved:
                    raise OSError("injected checkpoint write failure")
                result = original(src, dst)
                saved.append(pathlib.Path(dst).read_bytes())
                return result
            return original(src, dst)
        with patch.object(os, "replace", fail):
            receipt = self.execute()
        trouble = self.retained_trouble(receipt)
        self.assertTrue(trouble["persistence_errors"])
        checkpoint = next(self.remote_proof.glob("diagnostics/*/*/*/checkpoint.json"))
        self.assertEqual(checkpoint.read_bytes(), saved[0])
        self.assertEqual(trouble["observation"]["classification"], "scanner_completed")
        self.assertEqual(self.transaction(receipt)["observation"]["kind"], "INCONCLUSIVE")
    ...
    def test_frozen_input_mismatch_refuses_before_reservation(self):
        cases = ((self.command, "source_mismatch", True),
                 ...
                 (self.runtime_observation, "baseline_contract_sha256", "0" * 64))
