#!/usr/bin/env python3
"""Non-real-time ShieldXL evdev reader with stable typed output."""
...
DEVICES = tuple(
    Device("encoder", index, Path(f"/dev/input/shieldxl-encoder-{index + 1}"), REL_X)
    for index in range(3)
) + tuple(
    Device("button", index, Path(f"/dev/input/shieldxl-button-{index + 1}"), BTN_0 + index)
    for index in range(3)
)
