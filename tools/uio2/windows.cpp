...
struct View final:CPluginView {
  ...
  static LRESULT CALLBACK proc(HWND w,UINT m,WPARAM a,LPARAM b){
    ...
    if(v){
      ...
      if(m==WM_LBUTTONUP){inc(128);
        ...
        else if(w==v->popup&&x>=0&&x<240&&y>=0&&y<80){DestroyWindow(v->popup);v->popup=nullptr;put(56,0);v->request();}
        return 0;
      }
  ...
  tresult PLUGIN_API removed() override {
    for(auto w:{popup,lookalike,foreign,child})if(w)check(DestroyWindow(w)!=0,"fixture child cleanup");
...
int wmain(int argc,wchar_t** argv){try{
  ...
  }else{const auto end=GetTickCount64()+120000;while(GetTickCount64()<end&&get(80)!=9){host.pump();controller.view->measure();Sleep(5);}check(get(80)==9,"fixture deadline");}
