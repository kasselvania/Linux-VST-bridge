"""One generated headless owned-popup/SDK resize fixture; run in its own bounded systemd unit.
...
sys.path.insert(0, str(Path(__file__).resolve().parents[1] / 'uir1'))
...
def main():
    ...
    command = ['dbus-run-session', '--', 'kwin_wayland', '--virtual', '--xwayland',
               '--socket', 'uir1-isolated', '--width', '1280', '--height', '800',
