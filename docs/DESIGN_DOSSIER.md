...
### AP15 local checkpoint

The active editor-lifecycle work now has explicit native-view/editor-epoch/activation identities and a versioned GUI close contract. Its unmapped noninteractive X11 delegate remains an experiment until actual Bitwig open/close/reopen behavior is demonstrated. The distinct engineering qualification route reuses AP14 publication/rollback and preserves the verified revision-3 parent. Its exact compiled candidate roster currently refuses because the Windows artifact is pending; ordinary activation authority is unchanged. GitHub's account billing/spending gate is an external CI-start condition, not a repository test failure or acceptance result. See [AP15](AP15.md) for the bounded local checkpoint and remaining exact-fixture obligations.
