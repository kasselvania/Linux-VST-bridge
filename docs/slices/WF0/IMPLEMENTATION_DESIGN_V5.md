# Implementation Design Card — WF0 V5
...
V5 preserves the immutable V4 card and repairs its one independently reviewed
lifecycle-order authority gap:

```yaml
superseded_design_revision: wf0-design-v4
superseded_design_commit: 566625445c1b3dca3a253da54756513b0c5862e3
superseded_design_tree: 9e03cc47caf044237310a9ffd155c4d908142894
superseded_design_blob: 6701ddf068dc1853ad746b15df413fc12fe095fd
superseded_design_sha256: 138f545ff28f9d1f4f76e049d4d0fa30186bfcebae627f18140b0be0eb55ad09
superseded_review_commit: 62cbf973f2e107f91191fc0dee90188d73b50423
superseded_review_tree: 8142bc714ac7b757fe17444d7c76c1242afec1ce
superseded_review_path: docs/slices/WF0/ADVERSARIAL_DESIGN_REVIEW_V4.md
superseded_review_blob: cdb3bdbc743586dd8fba9b693330a28a177bdac0
superseded_review_sha256: 3404119a3076070216506004e4ab89c51d23fef59285d519626dda29d023bd73
superseded_review_result: DESIGN_REPAIR_REQUIRED
superseded_review_findings: 1
repair_scope: distinguish required-export resolution and check from later lifecycle publication
```

V4 correctly carried both binding V2 implementation clarifications, but its
transition law required publishing `factory_export_found` before optional
entry handling while its retained 23-state lifecycle required the entry result
first. V5 changes only that conflict by distinguishing successful required-
export resolution and check from later publication of its lifecycle result.
The V4 card and review remain immutable and receive no retroactive repair.

V3 preserved the immutable approved V2 card and superseded only its
current-fixture binding:

```yaml
inherited_v2_design_revision: wf0-design-v2
inherited_v2_design_commit: 4a04d5b52d1fa8e2d309ed1e2883ff96a7963ca6
inherited_v2_design_tree: b14b216ad5964ec68a1cf32d33c4201670d3ffd7
inherited_v2_design_blob: d618cbf6b397f10947d50fd4824cd3e06ef55726
inherited_v2_design_sha256: f323e2b429c4f91c1821488d980b249901b8d82c3cceaa7bf9be9d874a455e24
inherited_v2_review: GitHub 5082923871 / DESIGN_CLEAR
inherited_v2_approval_blob: a84df2044b8a78b44b7c004b53b620d474994221
v3_superseding_trigger: WF0_PROTECTED_FIXTURE_DRIFT
```

The V2 card in `docs/slices/WF0/IMPLEMENTATION_DESIGN.md`, its review, and its
approval receipt remain immutable historical authority. V5 does not edit or
reinterpret them. V5 retains the exact Bitwig `6.1` reconciliation and every
other approved or inherited V2 through V4 law.
...
After a future independent review, this V5 card remains immutable. Review
disposition and approval belong to new separate records; neither is folded
into or used to mutate this card.

This card proposes a repaired design amendment. It does not approve itself. The
...
exact V5 design-approval receipt are accepted and `CURRENT_SLICE.md` is changed
by that later authority.

### Binding inherited implementation clarifications

V5 normatively carries forward both clauses bound by the V2 review and
approval. They apply to the complete V5 design and are not historical context:

1. Immediately after every ordinary return from any of the 15 closed call
   operations, capture the bounded return value, emit the exact paired
   `call_completed`, and synchronously flush it. No later lifecycle event may
   be emitted and no later call attempt may begin before that flush completes.
   In retained attribution, “in flight” means no validated completion record
   was observed; it is not instruction-level knowledge after abnormal process
   death.
2. Resolve and check required `GetPluginFactory` before invoking optional
   `InitDll`. If the required export is absent, emit the approved
   `factory_export_missing` result, do not invoke `InitDll`, perform only the
   applicable loaded-module cleanup, and retain `WF0_FACTORY_GET_BLOCKED`.
   If the export is present, retain that resolution internally without yet
   publishing `factory_export_found`; complete optional entry handling and its
   immediately flushed call completion, emit the entry lifecycle result, then
   emit `factory_export_found`, and only afterward attempt the already-resolved
   factory export when entry was absent or succeeded. Optional `ExitDll`
   cleanup remains governed by the loaded-module cleanup law.

The future V5 approval receipt and implementation handoff must quote or bind
these two clauses explicitly. They create no new owner, state, call operation,
path, proof row, blocker, mutation, or claim.
...
fixture. WF0 does not launch Bitwig and V5 does not claim that HP0 or HP1 has
...
| What changed in the Bitwig protected fixture? | application version `6.0.11` -> `6.1` and app commit `7b66aed37386ffff99e40bbd486f90ceee7ac1d5c59508c7952094122cebf50e` -> `8a048e733e74dda8b897339436153a2d5df952f29d362dfcaca9d8e0d6f6c231`; current ref is stable | bounded amendment reconciliation | bind the current app identity separately from historical proof packets | no |
| Did Bitwig runtime/configuration posture also drift? | no; runtime commit, scope, user-shadow absence, and user/system override hashes remain exact | bounded amendment reconciliation | preserve those exact live identities before/after WF0 | future drift only |
...
| Does V5 require another owner, path, proof row, or blocker? | no | owner/path/proof audit | use existing `ProtectedFixtureSnapshot`, implementation paths, row 34, and `WF0_PROTECTED_FIXTURE_DRIFT` | no |
...
The record must be fully visible in the pipe before control enters the call. A write or flush failure prevents the call and becomes `WF0_OUTPUT_NORMALIZATION_BLOCKED`. Immediately after every ordinary success or ordinary error return, the scanner captures the bounded return value, emits exactly one paired `call_completed` record, and synchronously flushes it. The completion carries a new record `sequence`, the original `attempt_sequence`, the same closed operation/interface/ordinal/tier tuple, a closed return kind, and bounded raw result fields. No later lifecycle event may be emitted and no later call attempt may begin before that completion flush succeeds. It never retains a pointer or free-form status. A missing validated completion means the attempt is retained as in flight; that term does not claim instruction-level knowledge after abnormal process death.
...
The success path retains the 23-state ordering above, selects one entry branch, `factory_export_found`, `factory_obtained`, one exit branch, and ends in `scanner_completed`. Each lifecycle result is causally backed by the applicable completed attempt or exact export-resolution observation: `module_entry_absent` is backed by optional-entry lookup, while `factory_export_missing` and `factory_export_found` are backed by the earlier required-export resolution and check. On timeout, exception, NTSTATUS exit, signal, malformed termination, or output closure, the supervisor uses the last fully validated `call_started` record without a completion; only when no attempt is in flight may it fall back to the last validated lifecycle state. Cleanup attempts may add a secondary failure but never replace an earlier primary failure.
...
4. Resolve and check required `GetPluginFactory` first with exact case. Export lookup does not call module code. If the required export is absent, resolve optional `ExitDll` solely for the applicable loaded-module cleanup, emit `factory_export_missing`, do not resolve or invoke `InitDll`, retain `WF0_FACTORY_GET_BLOCKED`, and proceed only through step 11 and step 12 cleanup. If the required export is present, retain the callable address and successful resolution internally without yet emitting `factory_export_found`; then resolve optional `InitDll` and optional `ExitDll` with exact case. No entry call may precede successful required-export resolution and check.
5. Only after required `GetPluginFactory` is known present, if `InitDll` exists, emit and flush `call_started(init_dll)`, call it once, capture the bounded result, emit and synchronously flush the paired completion before any lifecycle event, and then emit the entry lifecycle result. Absence emits `module_entry_absent` and does not block. A `false` return emits `module_entry_failed`, retains `WF0_MODULE_ENTRY_BLOCKED`, and prohibits a factory call. After the selected entry lifecycle result, emit `factory_export_found` for the already-retained successful export resolution. Thus the lifecycle publication order remains `module_entry_*` then `factory_export_found`; this later publication does not mean the export was resolved after entry.
6. Only if entry is absent or succeeds, after `factory_export_found`, emit and flush `call_started(get_plugin_factory)`, call the already-resolved required export once, capture the bounded result, emit and synchronously flush the paired completion before any lifecycle event, and require a non-null `IPluginFactory*`. If entry failed, proceed through applicable cleanup without attempting `GetPluginFactory`.
...
The three required-export/entry branches are closed and non-overlapping:

- missing required export: `factory_export_missing`, no `InitDll` resolution or
  invocation, then applicable `ExitDll` and `FreeLibrary` cleanup with
  `WF0_FACTORY_GET_BLOCKED` primary;
- present required export with entry absent or successful: selected
  `module_entry_*`, then `factory_export_found`, then the
  `get_plugin_factory` attempt; and
- present required export with entry failure: `module_entry_failed`, then
  `factory_export_found`, no `get_plugin_factory` attempt, then applicable
  cleanup with `WF0_MODULE_ENTRY_BLOCKED` primary.
...
This V5 design-amendment branch may differ from amendment basis
...
docs/slices/WF0/IMPLEMENTATION_DESIGN_V4.md
docs/slices/WF0/ADVERSARIAL_DESIGN_REVIEW_V4.md
docs/slices/WF0/IMPLEMENTATION_DESIGN_V5.md
docs/slices/WF0/ADVERSARIAL_DESIGN_REVIEW_V5.md
```

The immutable V3 design commit contains only the first three paths. Its next
commit adds only the fresh V3 review record. The V4 repair commit changes only
`CURRENT_SLICE.md` and adds the complete V4 card. Its next commit adds only the
fresh V4 review record. The V5 repair commit changes only `CURRENT_SLICE.md` and
adds this complete V5 card. A later review commit may add only the fresh V5
review record after reviewing the immutable V5 repair commit. A future approval
receipt is not part of any design or review commit and requires separate exact
operator approval. No implementation path above is authorized on this branch.
...
The inherited call-order clarifications are binding proof law across all 15
closed operations. Rows 14 through 18 and 25 through 30 must reject a timeline
that does not emit and synchronously flush the paired `call_completed`
immediately after an ordinary return and before any later lifecycle event or
call attempt. Row 15's missing-required-export case must also prove that
`InitDll` was not invoked; row 18 must prove that `InitDll` is reachable only
after successful required-export resolution and check, while also proving that
the retained lifecycle publication order is entry result then
`factory_export_found`. It must not use the later lifecycle event as evidence
that the earlier export check occurred. Cleanup attribution and precedence
remain required even when the primary result is `WF0_FACTORY_GET_BLOCKED`.
...
```yaml
review_path: docs/slices/WF0/ADVERSARIAL_DESIGN_REVIEW_V5.md
...
prior_v4_design_commit: 566625445c1b3dca3a253da54756513b0c5862e3
prior_v4_design_blob: 6701ddf068dc1853ad746b15df413fc12fe095fd
prior_v4_review_path: docs/slices/WF0/ADVERSARIAL_DESIGN_REVIEW_V4.md
prior_v4_review_blob: cdb3bdbc743586dd8fba9b693330a28a177bdac0
prior_v4_review_result: DESIGN_REPAIR_REQUIRED
prior_v4_findings_repaired: 1
prior_v3_review_path: docs/slices/WF0/ADVERSARIAL_DESIGN_REVIEW_V3.md
prior_v3_review_blob: 62c0b953ed6c9eda7724de53f7dd87f9a55d613f
prior_v3_review_result: DESIGN_REPAIR_REQUIRED
prior_v2_review_github_id: 5082923871
prior_v2_review_result: DESIGN_CLEAR
unresolved_findings: fresh independent V5 adversarial review required
```

The V4 review remains immutable and does not approve V5. A fresh independent
reviewer must inspect the exact V5 commit, tree, Git blob, SHA-256, the complete
V2 through V4 lineage inherited by this card, and the live reconciliation.
...
5. the primary claim, owner/state/process model, mutation envelope, 40 paths,
   35 proof rows, 23 blockers, and nonclaims remain unchanged; and
6. all 15 operations retain immediate paired completion publication and the
   required export is resolved and checked before optional entry;
7. missing export, present-export entry failure, and present-export entry
   absent/success each have one conforming lifecycle and cleanup timeline, with
   the entry result published before `factory_export_found` on every present
   branch; and
8. the resolution/check-versus-publication distinction, cleanup precedence,
   proof rows 14 through 18 and 25 through 30, future approval receipt, and
   implementation handoff are mutually consistent and binding.
...
```yaml
card_path: docs/slices/WF0/IMPLEMENTATION_DESIGN_V5.md
...
amendment_basis_tree: a6e2fc8c7a564f50e9033094fde847387478de62
fixture_reconciliation_path: docs/slices/WF0/BITWIG_6_1_FIXTURE_RECONCILIATION.md
approval_receipt_path: docs/slices/WF0/DESIGN_APPROVAL_V5.md
approval_receipt_identity: null
implementation_authorized: false
```

No V5 design approval exists. A later accepted approval receipt must bind this
...
23-result taxonomy, both binding inherited implementation clarifications, and
the distinction between successful required-export resolution/check and later
`factory_export_found` lifecycle publication.
`CURRENT_SLICE.md` must separately grant implementation authority before any
...
explicit nonclaims, PR posture, immediate synchronous completion-publication
law for all 15 closed operations, required-`GetPluginFactory`-resolution-before-
optional-`InitDll` law, missing-export no-entry law, and entry-result-before-
`factory_export_found` publication law. The handoff must bind both inherited
clarifications and the resolution/publication distinction verbatim or by exact
V5 identity. The paused implementation branch may then be
advanced only by the separately authorized authority transition. The handoff
