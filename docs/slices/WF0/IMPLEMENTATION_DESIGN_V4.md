# Implementation Design Card — WF0 V4
...
V4 preserves the immutable V3 card and repairs its one independently reviewed
authority gap:

```yaml
superseded_design_revision: wf0-design-v3
superseded_design_commit: 158e1229d1e3b6e84fbb05fc472ebf7f75887965
superseded_design_tree: 2c312d069cc54cefb3b382dc87638d3fa1dc82fa
superseded_design_blob: 523d81f4b449ff96251dd1e52abb741b20033f95
superseded_design_sha256: 948fbc249e399de82db8f7cd50f46a357e32be67ccdb7598b4d69183b921fc78
superseded_review_commit: b6a697d71ea1cf81974199cdd35d4b7dcede6d45
superseded_review_tree: 0182c76f39567147da1c6767e3f0e2ac8eafc7e1
superseded_review_path: docs/slices/WF0/ADVERSARIAL_DESIGN_REVIEW_V3.md
superseded_review_blob: 62c0b953ed6c9eda7724de53f7dd87f9a55d613f
superseded_review_sha256: 08284b2f8f5f7b11b15dbf8eec3b30494e64539e0f5bc756b7a6e5c71f387a36
superseded_review_result: DESIGN_REPAIR_REQUIRED
superseded_review_findings: 1
repair_scope: preserve both binding V2 implementation clarifications normatively
```

V3 correctly incorporated the Bitwig `6.1` stable-fixture reconciliation, but
its complete successor contract did not unambiguously restate both binding V2
implementation clarifications. V4 changes only that omission. The V3 card and
review remain immutable and receive no retroactive repair.
...
approval receipt remain immutable historical authority. V4 does not edit or
reinterpret them. V4 retains the exact Bitwig `6.1` reconciliation and every
other approved V2/V3 law.
...
After a future independent review, this V4 card remains immutable. Review
...
exact V4 design-approval receipt are accepted and `CURRENT_SLICE.md` is changed
by that later authority.

### Binding inherited implementation clarifications

V4 normatively carries forward both clauses bound by the V2 review and
approval. They apply to the complete V4 design and are not historical context:
...
2. Resolve and check required `GetPluginFactory` before invoking optional
   ...
   applicable loaded-module cleanup, and retain
   `WF0_FACTORY_GET_BLOCKED`. Optional `ExitDll` cleanup remains governed by
   the loaded-module cleanup law.

The future V4 approval receipt and implementation handoff must quote or bind
...
fixture. WF0 does not launch Bitwig and V4 does not claim that HP0 or HP1 has
...
| Does V4 require another owner, path, proof row, or blocker? | no | owner/path/proof audit | use existing `ProtectedFixtureSnapshot`, implementation paths, row 34, and `WF0_PROTECTED_FIXTURE_DRIFT` | no |
...
4. Resolve and check required `GetPluginFactory` first with exact case. If it is absent, emit `factory_export_missing`, do not invoke `InitDll`, retain `WF0_FACTORY_GET_BLOCKED`, and proceed only through applicable loaded-module cleanup. Optional `ExitDll` discovery and cleanup remain governed by step 11. No entry call may precede proof that the required factory export exists. When the required export exists, emit `factory_export_found`, then resolve optional `InitDll` and optional `ExitDll` with exact case. Export lookup does not call module code.
5. Only after required `GetPluginFactory` is known present, if `InitDll` exists, emit and flush `call_started(init_dll)`, call it once, capture the bounded result, emit and synchronously flush the paired completion before any lifecycle event, and then retain the entry lifecycle result. `false` is `module_entry_failed`; do not call the factory. Absence is `module_entry_absent` and does not block.
6. If entry is absent or succeeds, emit and flush `call_started(get_plugin_factory)`, call the already-resolved required export once, capture the bounded result, emit and synchronously flush the paired completion before any lifecycle event, and require a non-null `IPluginFactory*`.
...
This V4 design-amendment branch may differ from amendment basis
...
`CURRENT_SLICE.md` and adds this complete V4 card. A later review commit may add
only the fresh V4 review record after reviewing the immutable V4 repair commit.
A future approval receipt is not part of any design or review commit and
requires separate exact operator approval. No implementation path above is
...
after `factory_export_found`. Cleanup attribution and precedence remain
required even when the primary result is `WF0_FACTORY_GET_BLOCKED`.
...
```yaml
review_path: docs/slices/WF0/ADVERSARIAL_DESIGN_REVIEW_V4.md
...
prior_v3_findings_repaired: 1
prior_v2_review_github_id: 5082923871
prior_v2_review_result: DESIGN_CLEAR
unresolved_findings: fresh independent V4 adversarial review required
```

The V3 review remains immutable and does not approve V4. A fresh independent
reviewer must inspect the exact V4 commit, tree, Git blob, SHA-256, the complete
V3 and V2 lineage inherited by this card, and the live reconciliation.
...
6. both inherited V2 clarifications are normative across all 15 call
   completions, required-export/optional-entry order, cleanup precedence, proof
   rows 14–18 and 25–30, the future approval receipt, and the implementation
   handoff.
...
```yaml
card_path: docs/slices/WF0/IMPLEMENTATION_DESIGN_V4.md
...
amendment_basis_tree: a6e2fc8c7a564f50e9033094fde847387478de62
fixture_reconciliation_path: docs/slices/WF0/BITWIG_6_1_FIXTURE_RECONCILIATION.md
approval_receipt_path: docs/slices/WF0/DESIGN_APPROVAL_V4.md
approval_receipt_identity: null
implementation_authorized: false
```

No V4 design approval exists. A later accepted approval receipt must bind this
...
23-result taxonomy, and both binding inherited implementation clarifications.
...
law for all 15 closed operations, and required-`GetPluginFactory`-before-
optional-`InitDll` law. The handoff must bind both inherited clarifications
verbatim or by exact V4 identity. The paused implementation branch may then be
