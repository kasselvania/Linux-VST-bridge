...
V2 supersedes the exact reviewed V1 card:
...
After a future independent review, this V2 card remains immutable. Review disposition and approval belong to separate records; neither is folded into or used to mutate this card.
...
The V2 repair commit changes only `docs/slices/WF0/IMPLEMENTATION_DESIGN.md` and `docs/slices/WF0/ADVERSARIAL_DESIGN_REVIEW.md`. No implementation path above is authorized on this branch.
...
The V2 matrix has 35 rows because the six added causal/source ownerships cannot truthfully be inferred from the V1 rows. One production run may satisfy several rows only where its receipts map them explicitly; no target test count is implied.
...
| 34 | accepted WR0/protected state unchanged | before/after equality and mutation-root audit | deliberate protected-snapshot mismatch blocks packet | live safe readback | yes, protection/evidence | `PRESERVATION.md` | no secret-content inspection |
...
```yaml
review_path: docs/slices/WF0/ADVERSARIAL_DESIGN_REVIEW.md
reviewed_revision: wf0-design-v1
reviewed_commit: aa07e63368dddc2fdb51242326a0de127c6ffbee
github_review_id: 5082639684
review_result: DESIGN_REPAIR_REQUIRED
findings_resolved_in_v2: 4
v2_review_result: review_required
unresolved_findings: fresh independent V2 adversarial review required
```

The review file materializes the supplied independent GitHub review; the design-authoring agent did not independently review or approve its own card. V2 resolves the four supplied findings but does not approve itself. A fresh independent reviewer must review the exact V2 Git blob/SHA-256 and record that disposition separately.
...
No design approval exists. A later accepted approval receipt must bind this exact revision, Git blob, SHA-256, selection basis, primary claim, 40-path envelope, 35-row proof matrix, and 23-result taxonomy. `CURRENT_SLICE.md` must separately grant implementation authority before any implementation action.
