...
Blackhole, Kontakt, Pigments, Pure LoFi and Serum 2 have **no accepted Ubuntu product row**. Deck results do not transfer to Ubuntu. The Raspberry Pi standalone experiment is a separate appliance claim, not a native-DAW support row here. The [PSL1 selected-state candidate](../evidence/psl1/serum-selected-state-2026-09-25.md) now covers exact Serum 2.1.5 A/B/C preparation and explicit in-session state changes on Pi 5. Its untraced A-to-B-to-C run had zero missing frames, xruns or faults; a traced transition run retained a later 256-frame gap. It is uninstalled and does not establish ordinary preset-selection UX, physical listening, reboot recall or general musical support.
