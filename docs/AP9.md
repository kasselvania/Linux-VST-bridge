...
## Owner-cost follow-up

Review 5127122244 led to a narrow stat-only process-tracking change. The matched eight-note workload reduced Python CPU by 57% and the complete owner cohort by 22%, with supervision cadence and audio binaries unchanged. A new focused Bitwig comparison retains 512 as recommended: 256 still missed frames. The 512 playback had zero bridge gaps but a separately recorded Bitwig close-time engine failure. See the [follow-up report](AP9_OWNER_COST.md) and [measurements](../evidence/AP9/owner-cost.json); the original results above remain unchanged.
