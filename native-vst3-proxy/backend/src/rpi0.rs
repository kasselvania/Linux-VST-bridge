//! Standalone ARM entry surface over the shared AP10/AP11 backend.
//!
...
impl Instance {
    ...
    pub fn processing_quantum(&self) -> u32 {
        self.quantum
    }
    ...
    pub fn setup(&self, maximum: u32, rate: f64, buses: &[u8]) -> io::Result<ProcessingTraits> {
        let mut traits = [0; 3];
        let code = unsafe {
            queued::ap10_setup(
                self.handle,
        ...
        result(code, "setup")?;
        self.negotiated_maximum.store(
            maximum.min(self.quantum),
