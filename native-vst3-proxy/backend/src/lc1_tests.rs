...
use super::*;
use std::{fs, process::{Command, Child, Stdio}, path::PathBuf};

struct Run { child: Child, handle: u64 }
impl Drop for Run {
    fn drop(&mut self) {
        if self.handle != 0 { unsafe { ap3_close(self.handle); } }
...
fn wait(shared: &Shared, condition: impl Fn() -> bool) {
    ...
    while !condition() {
        assert_eq!(shared.fault.load(Ordering::Acquire),0,"LC1 worker: {:?}",shared.detail.lock().unwrap());
...
fn same_session_reconfiguration_two_ended() {
    let root=PathBuf::from(std::env::var_os("LVB_LC1_DIRECTORY").expect("private empty test directory"));
    ...
    let child=Command::new(launcher).arg(&root).arg("1c".repeat(16))
    ...
    let mut shared=Shared::new();shared.gui=session.gui.clone();
    shared.state_capable.store(true,Ordering::Release);shared.ack.store(17,Ordering::Release);
    let shared=Arc::new(shared);let peer=shared.clone();
    let worker=thread::spawn(move || super::worker(session,peer,None));
    run.handle=INSTANCES.insert(|| Ok::<_,io::Error>(Live{
        shared:shared.clone(),callback:UnsafeCell::new(Callback::new()),busy:AtomicBool::new(false),
        worker:Some(worker),report:None,max:256,recovery_blocked:false,installed_delay:Some(512),minor:12,setup:None,
    })).unwrap().unwrap();
    let handle=run.handle;
    let mut setup=crate::performance::wire(256,0,48000.).unwrap();
    setup[20..24].copy_from_slice(&1u32.to_le_bytes());setup.extend(2u32.to_le_bytes());
    ...
    for epoch in 1..=2 {
        ...
        if epoch==2 {assert!(unsafe{control(handle,20,setup.clone())}.is_ok());}
        ...
        for block in 0..blocks {
            ...
            let mut request=Item::control(AUDIO,0);request.n=256;request.gain=0.5;
            request.data=[[0.25;CAP],[-0.5;CAP]];
            let mut out=[[0.;CAP];2];
            assert_eq!(cb.epoch,epoch);assert_eq!(cb.position,block*256);
            cb.process(&shared,request,&mut out).unwrap();
            drop(live);
            let expected=if epoch==1{block+1}else{4};
            wait(&shared,||shared.processed.load(Ordering::Acquire)==expected);
    ...
    assert_eq!(unsafe{ap3_close(handle)},0);run.handle=0;
    let end=Instant::now()+Duration::from_secs(10);
    loop {
        if let Some(status)=run.child.try_wait().unwrap(){assert!(status.success());break;}
        assert!(Instant::now()<end,"Windows fixture exit deadline");thread::sleep(Duration::from_millis(10));
    }
    eprintln!("LC1 two intervals passed: Start2 sequence104687 epoch2 position0, four exact SDK results");
}
