...
The two JSON files in this directory implement schema 1 with closed capabilities and exact local-fixture constraints. Both are immutable revision 2 candidates, with revision 1 retained in Git and installed publication history. Both remain `review_candidate`; neither is a universal Arturia support claim. The manager embeds this reviewed candidate set; arbitrary diagnostic profile input cannot activate a publication. See [implementation and verification](../docs/AP14.md).
