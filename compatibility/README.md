...
AP15 revision 4 is retained under `ap15/revision-4/` with its exact initial artifacts. Actual Bitwig rejected its 1×1/absent-child attachment. The sealed engineering roster now contains revision-5 ReviewCandidates with the narrow native attachment repair; neither candidate revision is ordinary activation authority. Revision 3 remains the exact verified rollback parent.

Revision 5 is retained byte-for-byte under `ap15/revision-5/`. It satisfied native attachment, same-session LoFi vendor close and one-action reopen, but the reopened editor failed foreground activation. Revision-6 ReviewCandidates bind the native-only unknown-timestamp activation repair and remain engineering-only; actual focus qualification is pending. The original Windows artifact and verified revision-3 parents remain exact.
