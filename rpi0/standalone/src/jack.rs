...
impl Control {
    ...
    pub fn set_output_trim(&self, trim: f32) -> io::Result<()> {
        if !trim.is_finite() || !(0.0..=1.0).contains(&trim) {
            return Err(invalid(
                "output trim must be finite linear gain from 0 to 1",
            ));
        }
        self.output_trim_bits
            .store(trim.to_bits(), Ordering::Relaxed);
        Ok(())
    }
...
impl Client {
    ...
    pub fn open_with_io(
        ...
        let inputs = if stereo_input {
            Some([
                register("audio_in_l", JACK_DEFAULT_AUDIO_TYPE, JACK_PORT_IS_INPUT)?,
    ...
    pub fn audio_input_ports(&self) -> Option<[String; 2]> {
        self.rt.inputs.map(|ports| {
            ports.map(|port| unsafe {
                std::ffi::CStr::from_ptr(jack_port_name(port))
                    .to_string_lossy()
                    .into_owned()
            })
        })
...
unsafe extern "C" fn process(frames: JackNFrames, argument: *mut c_void) -> c_int {
    ...
    let midi_buffer = if rt.midi.is_null() {
        ptr::null_mut()
    } else {
        jack_port_get_buffer(rt.midi, frames)
    };
    let midi_count = if midi_buffer.is_null() {
        0
    } else {
    ...
    let (inputs, silence) = if let Some(ports) = rt.inputs {
        (
            [
                ...
                slice::from_raw_parts(jack_port_get_buffer(ports[1], frames).cast::<f32>(), count),
            ],
            0,
        )
    } else {
        ([&rt.zero[..count], &rt.zero[..count]], 3)
    };
