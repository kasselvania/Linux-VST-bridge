...
// Private experimental layout. Default builds retain the original map exactly.
pub const MAP_VERSION: u32 = if cfg!(feature = "rpi2-quantum") { 2 } else { 1 };
pub const CAP: usize = if cfg!(feature = "rpi2-quantum") { 512 } else { 256 };
pub const STRIDE: usize = (CAP + 2) * 4;
pub const INPUT: usize = 64;
pub const OUTPUT: usize = INPUT + 2 * STRIDE;
pub const MAP_BYTES: usize = OUTPUT + 2 * STRIDE;
