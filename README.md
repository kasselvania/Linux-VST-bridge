...
AP15 local implementation is in draft PR #86. The native/GUI ownership changes and bounded engineering-qualification transaction have focused local tests, but the one-action Bitwig result is still pending. GitHub's billing/spending gate prevents the normal Windows build and CI jobs from starting. The candidate roster refuses activation until exact artifacts exist. The installed verified AP14 revision-3 devices, editor panel, service and 512-frame setting remain available. See [AP15 progress and limitations](docs/AP15.md#local-implementation-checkpoint--windows-qualification-pending).

AP15 remains a draft source-hardening checkpoint while Windows build capacity is unavailable. Its direct editor candidate is not installed or Bitwig-qualified. The current repair adds accepted-command retirement, safe timer/XCB failure handling, explicit editor callback origins and exact retained-qualification admission; see [AP15 source repair](evidence/ap15/source-repair/README.md). The installed verified revision-3 Arturia publications and 512-frame recommendation remain unchanged.
