...
## Implementation result awaiting review

AP6's explicit per-instance recovery and subsequent normal-desktop project recall passed the focused development check. The failed Windows endpoint was replaced from identified complete snapshot #3 while its sibling kept processing in the same native Bitwig host; neither setting was re-entered. Final-build recall reproduced both state hashes and audio. [AP6 results](docs/AP6_RESULT.md) record exact build/test provenance, cleanup, the snapshot-metadata notification repair and remaining limits. An initial native underflow and the historical AP5 transport loss remain unresolved reliability observations. This implementation is published for review; AP6 is not yet accepted or merged.
