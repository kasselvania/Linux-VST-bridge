...
impl eframe::App for DesktopFixture {
    fn ui(&mut self, ui: &mut egui::Ui, _frame: &mut eframe::Frame) {
        egui::CentralPanel::default().show(ui, |ui| {
            ...
            if ui.add_sized([240.0, 48.0], egui::Button::new("Test pointer / touch")).clicked() { self.count += 1; }
            ui.label(format!("Input receipts: {}", self.count));
            ui.label(format!("Display scale: {:.2}", ui.ctx().pixels_per_point()));
            if ui.add_sized([240.0, 48.0], egui::Button::new("Close fixture")).clicked() { ui.ctx().send_viewport_cmd(egui::ViewportCommand::Close); }
        });
    }
}
fn main() -> eframe::Result {
    ...
    eframe::run_native("Linux Audio Compatibility Manager",options,Box::new(|cc| {
        ...
        else {Ok(Box::new(operator::Operator::new(&cc.egui_ctx)))}
    }))
}
