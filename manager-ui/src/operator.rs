...
fn dependency_retirement_lines(v: &serde_json::Value) -> Vec<String> {
    ...
    if observation["schema"]==3 {
        let explanation=match observation["classification"].as_str() {
            ...
            Some("NAD2_STOP_SUBMITTED_NO_TRANSITION")=>"The stop request was submitted, but no SCM, exact-process, or listener transition was observed before the bound.",
        ...
        if observation["control_submitted"]==true {
            lines.push(format!("ControlService result: submitted in {} ms; returned state {}, controls mask {}, checkpoint {}, wait hint {} ms.",
        ...
        } else if !observation["control_error"].is_null() {
            lines.push(format!("ControlService result: not submitted; error {}.",observation["control_error"]));
        }
        ...
        for class in classes {
            let value=serde_json::json!({"dependency":{"service_retirement_confirmed":class=="NAD2_STOP_CONFIRMED",
                "process_cleanup_confirmed":false,"forced_cleanup_used":class!="NAD2_STOP_CONFIRMED","stop_observation":{
                    ...
                    "wait_hint_ms":0,"control_submitted":true,"control_error":0,"control_elapsed_ms":3,
            ...
            assert!(text.contains(class));assert!(text.contains("ControlService result: submitted"));
